...
- **Press-to-toggle is the no-setup default.** On GNOME 46/47 Wayland without the portal and without the extension, Witticism uses an `org.gnome.Shell.GrabAccelerator`-based fallback that works with no installation. Because that API delivers only activation events, the hotkey behaves as press-to-toggle (press once to start recording, press again to stop) rather than hold-to-talk. This is degraded but immediately working, and diagnostics point the user at the optional extension for the full experience.
