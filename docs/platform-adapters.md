...
This document is the maintained runtime contract for desktop integration. ADR-002 records why the contract exists; [ADR-004](adr/004-gnome-keybinding-press-to-toggle.md) records the GNOME custom-keybinding press-to-toggle default and the consent-gated automatic typing that this document reflects.
...
| Wayland with GlobalShortcuts portal (KDE, GNOME 48+) | XDG GlobalShortcuts | hold-to-talk | Clipboard (optional consent-gated portal typing) | Supported |
| GNOME 46/47 Wayland, extension installed and running | Witticism GNOME Shell extension | hold-to-talk | Clipboard (optional consent-gated portal typing) | Supported |
| GNOME 46/47 Wayland, no extension | Standard GNOME custom keyboard shortcut | hold-to-talk (via key-repeat inference); press-to-toggle if key repeat is disabled | Clipboard (optional consent-gated portal typing) | Supported |
...
On Wayland, text delivery is clipboard-first by design: transcripts are copied for a plain `Ctrl+V`, with no permission prompt. Automatic typing through the Remote Desktop portal is available but strictly opt-in - it is never probed or started, and GNOME's permission dialog never appears, until the user enables it (see [ADR-004](adr/004-gnome-keybinding-press-to-toggle.md)).
...
`TextOutputAdapter` owns delivery only. X11 and Windows retain direct typing. Wayland copies the complete transcript to the clipboard by default; only after the user consents does it request keyboard-only Remote Desktop portal access to **type** the transcript, injecting each character as an X11 keysym press/release. It synthesizes no shift/modifier (keysyms carry case and symbols themselves). Failure, denial, and the unconsented default all keep output on the clipboard.
...
2. Wayland is clipboard-first by design: transcripts are copied to the clipboard for a manual paste (`Ctrl+V`, or `Ctrl+Shift+V` in a terminal). Automatic typing (keysym injection through a keyboard-only Remote Desktop portal session with a persisted restore token) is **off by default** and consent-gated; see "Automatic typing consent" below. It is never probed or started until the user opts in.
...
## Automatic typing consent

Automatic typing is off by default and strictly opt-in. Until the user consents, the Remote Desktop portal is never touched (no probe, no session, no dialog) and output is clipboard-only. Consent is captured through an in-app priming dialog - Witticism's own branded window that explains what will happen and warns that GNOME will show a system confirmation titled "Remote Desktop" that actually comes from Witticism and covers keyboard input only. The dialog is offered at most once, automatically, right after the first successful transcription (never at startup), and is always reachable manually from the tray ("Enable automatic typing..."). Only when the user opts in does Witticism start the portal session - the single place GNOME's dialog may appear.
...
If automatic typing is later revoked mid-session (the user turns it off from GNOME's system remote-desktop indicator), the adapter observes the portal session's `Closed` signal, drops to clipboard, resets consent to `unset`, and deletes the restore token, so the tray's "Enable automatic typing..." offer reappears and re-enabling runs the full consent flow again. A typing injection that fails because the session vanished degrades the same way without losing the transcript (it is copied to the clipboard before typing is attempted).

## Portal permissions and state

Remote Desktop requests keyboard access only: no screen, pointer, touchscreen, or portal clipboard access. When the user grants automatic typing, authorization uses persistent mode. Its opaque, single-use restore token is stored separately from user configuration with mode 0600 and atomically rotated. Tokens must never appear in logs, diagnostics, configuration exports, or issues.

A granted session is restored silently at startup from the saved token. If that token is gone or revoked, Witticism does not silently re-pop the system dialog; it falls back to clipboard and asks the user to re-enable automatic typing from the tray. If authorization is denied or disconnected, transcription continues and remains on the clipboard. The transcript intentionally remains on the clipboard after automatic typing; delayed clipboard restoration is unreliable across toolkits.

GNOME inhibits new Remote Desktop sessions while the desktop is locked. If a granted session cannot be restored, automatic typing remains degraded until the session is unlocked and re-enabled; shortcut capture remains independent.
...
`witticism-platform doctor` reports session type, desktop, portal capabilities (GlobalShortcuts and RemoteDesktop), the selected shortcut backend and its capability tier (hold-to-talk, press-to-toggle (fallback), or unavailable), whether Witticism's own GNOME custom keybinding is currently registered and whether GNOME key auto-repeat is enabled (which determines the keybinding tier), the selected output backend and its tier (portal typing, typing, or clipboard), the automatic typing consent state (unset/granted/declined), the extension installed/running state, and whether a portal restore grant exists. When a session can be improved it also prints a "how to improve" line, such as the opt-in extension install hint with its logout caveat on GNOME, or the optional automatic typing hint on Wayland.
...
Broken platform integration is surfaced on the tray icon itself, not just as a transient notification. A **degraded** health tier badges the idle icon amber and annotates the tooltip and the Status menu row with a short reason. It triggers when the hotkey adapter is not usable (FAILED/UNAVAILABLE, at startup or after a rebind), or when automatic typing was granted but is now broken (mid-session revocation, or a startup token-restore failure). It does **not** trigger for the designed clipboard default (consent unset/declined) or the working press-to-toggle fallback. Precedence, high to low: real errors (CUDA/GPU, model load) > the live recording indicator > degraded > normal idle - so an active recording still flashes and the degraded badge reappears when idle. The degraded state clears when the hotkey is usable again, when automatic typing is re-granted, or when the user declines after a revocation (choosing the clipboard default).
